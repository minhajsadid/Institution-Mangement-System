...
if(isset($_POST['delete'])){
  ...
  $bookPath=$_POST['deleteBook'];
    $sql="DELETE FROM tbnotice WHERE titile='$bookPath'";

    if ($conn->query($sql)){
      echo '<script>confirm("Notice Deleted Successfully")</script>';
          // 
          
    } 
    else{
        echo '<script>confirm("Notice Deleted Failed")</script>';
        echo "Error: ". $sql ."
    ". $conn->error;
    }
    $conn->close();
      ...
      <div class="modal-body">
        <form method="POST">
          <div class="form-group">
            <select class="form-control" name="deleteBook" required>
              <option value="">Please Select book</option>
              <?php  
            
             $sql="SELECT * FROM tbnotice";
